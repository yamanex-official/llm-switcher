...
対策案: マーカーコメント `# >>> llm-switcher >>>` 〜 `# <<< llm-switcher <<<` で
...
- 第6章: `~/.config/llm-switcher/`
- 第10章: `os.UserConfigDir()` 配下の `llm-switcher/`
...
`~/.config/llm-switcher/` は Linux では正しいが Windows/macOS では実際に作成されるパスと一致しない。
